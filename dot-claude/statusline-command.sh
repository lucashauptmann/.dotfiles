...
# persist session data for claude-swarm (survives status line hiding)
if [ -n "$WEZTERM_PANE" ] && [ -n "$session_id" ]; then
  _panes_dir="$HOME/.claude/swarm/panes"
  [ -d "$_panes_dir" ] || mkdir -p "$_panes_dir"
  _tmp="$_panes_dir/${WEZTERM_PANE}.json.tmp"
  printf '{"paneId":%s,"sessionId":"%s","cwd":"%s","model":"%s","remainingPercent":%s,"updatedAt":%s}\n' \
    "$WEZTERM_PANE" "$session_id" "$cwd" "$model" "${remaining:-0}" "$(date +%s)" > "$_tmp"
  mv -f "$_tmp" "$_panes_dir/${WEZTERM_PANE}.json"
fi
